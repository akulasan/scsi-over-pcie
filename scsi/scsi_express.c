...
static int pqi_device_queue_pair_array_alloc(struct pci_dev *pdev,
			...
			u16 n_iq_elements, u8 iq_element_size_over_16,
			u16 n_oq_elements, u8 oq_element_size_over_16)
{
	void *vaddr;
	dma_addr_t dhandle;
	int i, nqs_alloced = 0;
	int total_size = (n_iq_elements * iq_element_size_over_16 * 16) +
			(n_oq_elements * oq_element_size_over_16 * 16) +
			sizeof(u64) * 2;

	dev_warn(&pdev->dev, "zzz 1\n");

	*iq = kzalloc(sizeof(**iq) * num_queue_pairs, GFP_KERNEL);
	*oq = kzalloc(sizeof(**oq) * num_queue_pairs, GFP_KERNEL);
	dev_warn(&pdev->dev, "zzz 2 *iq = %p, *oq = %p, total_size = %d\n",
			*iq, *oq, total_size);
	if (!*iq || !*oq)
		goto bailout;

	for (i = 0; i < num_queue_pairs; i++) {
		dev_warn(&pdev->dev, "zzz top of loop, i = %d\n", i);
		vaddr = pci_alloc_consistent(pdev, total_size, &dhandle);
		dev_warn(&pdev->dev, "zzz 2 i = %d, vaddr = %p\n", i, vaddr);
		if (!vaddr)
			goto bailout;
		(*iq)[i].queue_vaddr = vaddr;
		(*iq)[i].dhandle = dhandle;
		(*iq)[i].ci = vaddr +
				iq_element_size_over_16 * 16 * n_iq_elements;
		/* (*iq)[i].pi is unknown now, hardware will tell us later */
		(*iq)[i].unposted_index = 0;
		(*iq)[i].element_size = iq_element_size_over_16 * 16;
		(*iq)[i].nelements = n_iq_elements;
		/* By convention, IQs have even queue ids: 4, 6, 8, etc.
		 * queue_id_of(corresponding_oq_of(IQ)) ==
		 * queue_id_of(IQ) + 1;
		 */
		(*iq)[i].queue_id = (i + 2) * 2;

		vaddr += iq_element_size_over_16 * 16 * n_iq_elements +
				sizeof(u64);
		dev_warn(&pdev->dev, "zzz 3 i = %d, vaddr = %p\n", i, vaddr);
		(*oq)[i].queue_vaddr = vaddr;
		(*oq)[i].dhandle = dhandle +
				oq_element_size_over_16 * 16 + sizeof(u64);
		/* (*oq)[i].ci is unknown now, hardware will tell us later */
		(*oq)[i].pi = vaddr +
				oq_element_size_over_16 * 16 * n_oq_elements;
		(*oq)[i].unposted_index = 0;
		(*oq)[i].element_size = oq_element_size_over_16 * 16;
		(*oq)[i].nelements = n_oq_elements;
		/* 
		 * By convention, OQs have odd queue ids: 5, 7, 9, etc.
		 * queue_id_of(corresponding_iq_of(OQ)) ==
		 * queue_id_of(OQ) - 1;
		 */
		(*oq)[i].queue_id = (i + 2) * 2 + 1;
		nqs_alloced++;
...
static int scsi_express_setup_io_queues(struct scsi_express_device *h)
{
	...
	nqs = pqi_device_queue_pair_array_alloc(h->pdev,
			&h->io_q_to_dev, &h->io_q_from_dev, MAX_IO_QUEUES,
			IQ_NELEMENTS, IQ_IU_SIZE / 16,
			OQ_NELEMENTS, OQ_IU_SIZE / 16);

	dev_warn(&h->pdev->dev, "%d queue pairs alloced\n", nqs);
	if (nqs < MAX_IO_QUEUES)
		dev_warn(&h->pdev->dev,
			"allocated only %d of %d i/o queue pairs\n",
			nqs, MAX_IO_QUEUES);
	if (nqs <= 0)
		goto bail_out;

	dev_warn(&h->pdev->dev, "Setting up %d io queues\n", nqs);
	for (i = 0; i < nqs; i++) {
		struct pqi_device_queue *q;

		dev_warn(&h->pdev->dev,
			"Set up io queue %d Qid(%d) from device\n", i,
		...
		dev_warn(&h->pdev->dev, "Writing 0x%x to address %p\n",
			h->admin_q_to_dev.unposted_index, h->admin_q_to_dev.pi);
